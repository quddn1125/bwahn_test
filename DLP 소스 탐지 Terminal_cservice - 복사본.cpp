...
void CService::ServiceMainMember(DWORD argc, LPTSTR* argv, LPHANDLER_FUNCTION pf, LPTHREAD_START_ROUTINE pfnWTP)
{
   ...
   __try
   {
	  ...
	  //아무것도 하지 않음.
      ParseArgs(argc, argv);
	  //Thread를 생성한 후 SCM에서 전달되는 Event를 기다려서 처리함.
	  //생성된 Thread는 CService::WatcherThreadMemberProc를 수행.
      LaunchWatcherThread(pfnWTP);
	  //초기화 작업수행.
	  // - 해당 서비스가 수행되고 있는 Computer의 Netbios명을 읽어옴.
      Init();
	  //SCM에 서비스가 정상적으로 수행하고 있다고 알리고 메인작업 수행(LOOP)
	  // - 필요한 Registry정보 읽기.
	  // - Listening Socket, IOCP초기화, Accept Clients Request.
      Run();
